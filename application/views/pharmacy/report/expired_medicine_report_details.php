...
<head>
    ...
    <title>Medicine Expired Report</title>
</head>

<body>
    <?php
    $total_expired_medicines = $this->db->where('date>=', date('Y-m-d', strtotime($from_date)))
        ->where('date<=', date('Y-m-d', strtotime($to_date)))
        ->get('expired_medicines');
    $total_rows = $total_expired_medicines->num_rows();
                ...
                <div class="second">
                    <table border="1" class="table table-bordered table-hover" style="width: 90%;margin: 0 auto;color:black;border-collapse:collapse;">
                        <tr style="background-color: #0074B3;color: white;">
                            <td colspan="6" style="text-align: center"><b>Expired Medicine Report</b> From date <b><?php echo date('d-m-Y', strtotime($from_date)); ?></b> To date <b><?php echo date('d-m-Y', strtotime($to_date)); ?></b></td>
                        </tr>
                        <tr>
                ...
                <div class="first">
                    <div class="second">
                        <table border="1" class="table table-bordered table-hover" style="width: 90%;margin: 0 auto;color:black;border-collapse:collapse;">
                            ...
                            $this->db->from('expired_medicines'); // Base table
                            $this->db->where('date >=', date('Y-m-d', strtotime($from_date))); // Date range filter
                            $this->db->where('date <=', date('Y-m-d', strtotime($to_date)));
                            $this->db->order_by('expired_medicine_id', 'DESC'); // Order by expired_medicine_id
                            $this->db->limit(30, $from); // Pagination limit and offset
                            $query_expired_medicines = $this->db->get();
                            $query = $query_expired_medicines->result();
                            ...
                            ?>
                                <tr>
                                    <td><?php echo $sl++ ?></td>
                                    <td><?php echo $query_value->expired_medicine_invoice_no ?></td>
                                    <td><?php echo $query_value->remarks ?></td>
                                    <td>
                                        <?php
