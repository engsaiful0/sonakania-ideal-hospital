...
<head>
    ...
    <title>Purpose Debit Voucher Report</title>
</head>
...
<body>
    ...
    $total_debit_account = $this->db->select('*')
        ->get('debit_account');
    $total_rows = $total_debit_account->num_rows();
                ...
                <div class="first">
                    <div class="second">
                        <table border="1" class="table table-bordered table-hover" style="width: 90%;margin: 0 auto;color:black;border-collapse:collapse;">
                            ...
                            <?php
                            $from = $from + 30;
                            $this->db->select('*'); // Select all columns from debit_voucher and account_name from credit_account
                            $this->db->from('debit_account'); // Base table
                            $this->db->limit(30, $from); // Pagination limit and offset
                            $query_debit_account = $this->db->get();
                            $query = $query_debit_account->result();
                            $sl = 1;
                            foreach ($query as $query_value) {

                                $this->db->select_sum('total_amount');
                                $this->db->from('debit_voucher');
                                $this->db->where('debit_account_id', $query_value->debit_account_id);
                                $this->db
                            ...
                            ?>
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        $grand_total_amount += $total_debit_voucher_today;
                                        ?>
                                    </td>
