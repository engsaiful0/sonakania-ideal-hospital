    ...
    <body>
        ...
        if ($doctor_id != '') {
            $total_test_reference = $this->db->where('date>=', date('Y-m-d', strtotime($from_date)))
                    ->where('reference_doctor_id', $doctor_id)
                    ->where('doctor_id!=', '')
                ...
                <div class="second">
                    <table border="1" border="1" style="width: 98%;border-collapse:collapse;margin:0 auto;color:black;margin-top: 30px;">
                        ...
                        if ($doctor_id != '') {
                            $from = $from + 45;
                            $this->db->select('*');
                            $this->db->order_by('reference_doctor_id', 'asc');
                        ...
                        foreach ($query as $query_value) {
                            ...
                            $doctor = $this->db->where('doctor_id', $query_value->reference_doctor_id)
                                            ->get('doctor')->row();
                            ?>
                            <tr>
                                <td>
