...
<head>
    ...
    <title>Canteen Ready Item Sell Report</title>
</head>

<body>
    <?php
    $total_canteen_ready_item_sell = $this->db->where('date>=', date('Y-m-d', strtotime($from_date)))
        ->where('date<=', date('Y-m-d', strtotime($to_date)))
        ->get('canteen_ready_item_sell');
    $total_rows = $total_canteen_ready_item_sell->num_rows();
                ...
                <div class="second">
                    <table border="1" class="table table-bordered table-hover" style="width: 90%;margin: 0 auto;color:black;border-collapse:collapse;">
                        <tr style="background-color: #0074B3;color: white;">
                            <td colspan="6" style="text-align: center"><b>Canteen Ready Item Sell Report</b> From date <b><?php echo date('d-m-Y', strtotime($from_date)); ?></b> To date <b><?php echo date('d-m-Y', strtotime($to_date)); ?></b></td>
                        </tr>
                        <tr>
            ...
            <page size="A4">
                <div class="first">
                    <div class="second">
                        <table border="1" class="table table-bordered table-hover" style="width: 90%;margin: 0 auto;color:black;border-collapse:collapse;">
                            <tr style="background-color: #0074B3;color: white;">
                                <td colspan="7" style="text-align: center"><b>Expired Medicine Report</b> From date <b><?php echo date('d-m-Y', strtotime($from_date)); ?></b> To date <b><?php echo date('d-m-Y', strtotime($to_date)); ?></b></td>
                            </tr>
                            <tr>
                                <td>Sl</td>
                                <td>Customer</td>
                            ...
                            $this->db->from('canteen_ready_item_sell'); // Base table
                            $this->db->where('date >=', date('Y-m-d', strtotime($from_date))); // Date range filter
                            $this->db->where('date <=', date('Y-m-d', strtotime($to_date)));
                            $this->db->order_by('canteen_ready_item_sell_id', 'DESC'); // Order by expired_medicine_id
                            $this->db->limit(30, $from); // Pagination limit and offset
                            $query_canteen_ready_item_sell = $this->db->get();
                            $query = $query_canteen_ready_item_sell->result();
                            ...
                            ?>
                                <tr>
                                    <td><?php echo $sl++ ?></td>
                                    <td><?php echo $query_value->customer_name ?></td>
                                    <td><?php echo $query_value->canteen_ready_item_sell_invoice_no ?></td>
                                    <td>
                                        <?php
                                    ...
                                    <td>
                                        ...
                                        $grand_paid += floatval($query_value->paid);
                                        ?>
                                    </td>
                                    <td>
                                        <?php
                                        echo $query_value->due;
                                        $grand_due += floatval($query_value->due);
                                        ?>
                                    </td>
