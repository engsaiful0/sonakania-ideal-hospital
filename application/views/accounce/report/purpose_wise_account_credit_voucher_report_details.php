...
<head>
    ...
    <title>Purpose Credit Voucher Report</title>
</head>

<body>
    <?php
    $total_credit_account = '';

    $total_credit_account = $this->db->select('*')
        ->get('credit_account');
    $total_rows = $total_credit_account->num_rows();
                    ...
                    <div class="second">
                        <table border="1" class="table table-bordered table-hover" style="width: 90%;margin: 0 auto;color:black;border-collapse:collapse;">
                            <tr style="background-color: #0074B3;color: white  ">
                                <td colspan="7" style="text-align: center"><b>Purpose Wise Credit Voucher Report</b> From date <b><?php echo date('d-m-Y', strtotime($from_date)); ?></b> To date <b><?php echo date('d-m-Y', strtotime($to_date)); ?></b></td>
                            </tr>
                            <tr>
                                <td>Sl</td>
                                <td>Purpose</td>
                            ...
                            <?php
                            $from = $from + 30;
                            $this->db->select('*'); // Select all columns from credit_voucher and account_name from credit_account
                            $this->db->from('credit_account'); // Base table
                            $this->db->limit(30, $from); // Pagination limit and offset
                            $query_credit_account = $this->db->get();
                            $query = $query_credit_account->result();
                            $sl = 1;
                            foreach ($query as $query_value) {

                                $this->db->select_sum('total_amount');
                                $this->db->from('credit_voucher');
                                $this->db->where('credit_account_id',$query_value->credit_account_id);
                                $this->db
                            ...
                            ?>
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        $grand_total_amount += $total_credit_voucher_today;
                                        ?>
                                    </td>
