...
?><option>Select Invoice</option>
    ...
    public function addMoreSale_edit()
    {
        ...
        <tr>
            ...
            <td style="padding:5px;">
                <input name="qty_edit[]" value="0" type="hidden">
            ...
            <td style="padding:5px;">
                <input type="button" onclick="SomeDeleteRowFunction(this)" style="width:50px" sequence=<?php echo $next_id ?> readonly id="add_more_<?php echo $next_id ?>" title="Click TO Remove" value="-">
    ...
    ?>
        <tr>
            ...
            <td style="padding:5px;">
                <input type="text" value="" id="re_order_qty<?php echo $next_id ?>" name="re_order_qty[]" class="form-control" sequence=<?php echo $next_id ?>>
            </td>
            <td style="padding:5px;">
            ...
            <td style="padding:5px;">
                <input type="text" value="" id="vat<?php echo $next_id ?>" style="width:40px;" name="vat[]" class="form-control" sequence=<?php echo $next_id ?> onkeyup="purchase_rate(this, event), getamount(this, event)">
            </td>
    ...
    public function add_drug_name()
    {
        ...
        $sql = $this->db->where('drug_type_id', $type_name_val)->where('status', 'active')->order_by('drug_name','ASC')->get('drug')->result();
        ?>
        <option value="">Select Drug</option>
        <?php
        foreach ($sql as $value) {
    ...
    public function add_drug_name_purchase()
    {
        ...
        $sql = $this->db->where('type', $type_name_val)->where('manufacturer', $supplier)->get('drug')->result();
        ?>
        <option value="">Select Drug</option>
        <?php
        foreach ($sql as $value) {
    ...
    function medicin_sell()
    {
        ...
        $page_data = array(
            'page_name' => 'pharmacy/medicin_sell',
            'page_title' => 'Pharmacy',
    ...
    function purchase_delete()
    {
        ...
        $this->db->where('purchase_id', $purchase_id)->delete('purchase');
        $this->db->where('purchase_id', $purchase_id)->delete('purchase_details');
        $data = array();
        $data['date_from'] = $this->input->post('date_from');
        $data['date_to'] = $this->input->post('date_to');
...
<?php
    ...
    public function edit_confirm_sale()
    {
        ...
        $next_customer_id = 0;
        $this->db->where('customer_id', $customer)->update('customer', $customer_data); //For new Customer
        //echo'<pre>';
        ...
        $this->db->where('customer_id', $customer_id)->delete('customer_account');
        $customer_account_data = array(
        ...
        /* Start Sales Income Statement */
        $this->db->where('customer', $customer_id)->delete('sales_income_statement');
        ...
        /* End Sales Income Statement */
        $this->db->where('sales_id', $sales_id)->update('sales', $sales);
        $delete_update = array('is_deleted' => '1');
        $this->db->where('sales_id', $sales_id)->update('sales_details', $delete_update);
        $purchase_details = array();
        $drug = $this->input->post('drug');
        $type_name = $this->input->post('type_name');
        $sales_rate = $this->input->post('sales_rate');
        ...
        if ($insert) {
            for ($loop = 0; $loop < count($drug); $loop++) {
                $drugid = $drug[$loop];
                $drug_stock = $this->db->where('drug_id', $drugid)->get('drug')->row();
                $purchase_quantity = $qty[$loop];

                $update = array(
                    'stock' => $drug_stock->stock - $qty_edit[$loop] + $purchase_quantity
                );
                $this->db->where('drug_id', $drugid)->update('drug', $update);
            }
        }
        //  if ($update_stock)
        //   echo "1";
        ...
        $page_data = array(
            'page_name' => 'pharmacy/sell_print',
            'page_title' => 'Add Medicin ',
            'sidebar' => 'pharmacy/pharmacy_sidebar'
        );
        $this->load->view('content', $page_data);
        //  $this->load->view('pharmacy/sell_print');
    }
    ...
    function confirm_sale()
    {
        ...
        $this->db->insert('customer', $new_customer_data);

        $next_customer_id = $this->db->insert_id();
    ...
    public function sells_print_individual($sales_id)
    {
        $data['sales_id'] = $sales_id;
        $this->load->view('pharmacy/sells_print_individual', $data, TRUE);
        $page_data = array(
            'page_name' => 'pharmacy/sells_print_individual',
            'page_title' => 'Sale Print',
            'sidebar' => 'pharmacy/pharmacy_sidebar'
        );
        $this->load->view('content', $page_data);
    }

    public function sells_delete($sales_id)
    {
        $sales = $this->db->where('sales_id', $sales_id)->get('sales_details')->result();
        foreach ($sales as $value) {

            $quantiy = $value->qty;

            $drug = $this->db->where('drug_id', $value->drug)->get('drug')->row();
            //echo '<pre>';
            //  var_dump($value->drug);
            //die;
            // var_dump($drug);
            $update = array(
                'quantity' => $drug->stock - $quantiy
            );
            $delete_update = array(
                'is_deleted' => '1'
            );
            $this->db->where('drug', $value->drug)->update('sales_details', $delete_update);
        }
        $delete_update = array(
            'is_deleted' => '1'
        );
        $this->db->where('sales_id', $sales_id)->update('sales', $delete_update);
        $sdata = array('deleted' => 'deleted');
        $this->session->set_userdata($sdata);

        $bill_no = '';
        $config['base_url'] = site_url('ProductController/sells_delete');
        $config['total_rows'] = $this->db->where('is_deleted', '0')->count_all('sales');
        $config['per_page'] = "50";
        $config["uri_segment"] = 3;
        $choice = $config["total_rows"] / $config["per_page"];
        $config["num_links"] = floor($choice);
        ...
        // get books list
        $data['detailsList'] = $this->ProductModel->get_sells_details($config["per_page"], $data['page'], NULL, $bill_no);

        $data['pagination'] = $this->pagination->create_links();
        $this->load->view('pharmacy/view_meidcin_sell', $data, true);
        $page_data = array(
            'page_name' => 'pharmacy/view_meidcin_sell',
    ...
    public function view_medicin_sell()
    {
        $bill_no = $this->input->post('bill_no');
        $config['base_url'] = site_url('ProductController/view_medicin_sell');
        $config['total_rows'] = $this->db->where('is_deleted', '0')->count_all('sales');
        $config['per_page'] = "50";
        $config["uri_segment"] = 3;
        $choice = $config["total_rows"] / $config["per_page"];
    ...
    {
        $stock_data = $this->load->view('pharmacy/stock_details', $_POST, TRUE);
        echo $stock_data;
    }
}
