...
class CanteenPurchaseController extends CI_Controller
{
    private $per_page;
    ...
    public function add_purchase_canteen_goods()
    {
        $page_data = array(
            'page_name' => 'canteen_purchase/add_purchase_canteen_goods',
    ...
    public function save_canteen_goods_purchase()
    {
        ...
        if ($this->input->is_ajax_request()) {
            ...
            for ($loop = 0; $loop < count($canteen_raw_goods_id); $loop++) {
                ...
                $this->db->where('canteen_raw_goods_id', $canteen_raw_goods_id[$loop])
                    ->update('canteen_raw_goods', ['price' => $price[$loop]]);
            }
    ...
    public function update_purchase_goods_data()
    {
        ...
        if ($this->input->is_ajax_request()) {
            // Get form data
            $canteen_purchase_good_id = $this->input->post('canteen_purchase_good_id');
            $canteen_purchase_goods_previous_data = $this->db->where('canteen_purchase_good_id', $canteen_purchase_good_id)->get('canteen_purchase_goods')->row();
            $data = array(
                'purchase_canteen_goods_invoice_no' => $this->input->post('purchase_canteen_goods_invoice_no'),
                'canteen_goods_supplier_id' => $this->input->post('canteen_goods_supplier_id'),
                'purpose' => $this->input->post('purpose'),
            ...
            for ($loop = 0; $loop < count($canteen_raw_goods_id); $loop++) {
                $purchase_goods_details[] = array(
                    ...
                    'quantity' => $quantity[$loop],
                    'total_amount' => $total_amount[$loop],
                    'user_id' => $canteen_purchase_goods_previous_data->user_id,
                );
                $this->db->where('canteen_raw_goods_id', $canteen_raw_goods_id[$loop])
                    ->update('canteen_raw_goods', ['price' => $price[$loop]]);
            }
