...
class IpdPatientController extends CI_Controller
{
    ...
    public function delete_ipd_patient_ajax()
  {
      ...
      $seat_update = array(
          'status' => 'Available',
          'ipd_patient_id' => '', // Release the patient from the seat
      );

      $this->db->where('cabin_id', $ipd_patient->cabin_id)->update('cabin', $seat_update);
      $this->db->where('bed_id', $ipd_patient->bed_id)->update('bed', $seat_update);
